...
fn parse_pair(pair: &str) -> (u32, u32) {
    ...
    let lhs: u32 = pair[0..dash].parse::<u32>().expect("Failed to parse to int");
    let rhs: u32 = pair[dash+1..].parse::<u32>().expect("Failed to parse to int");
    (lhs, rhs)
}
...
pub fn solution1() -> Result<(), Box<dyn Error + 'static>> {
    let answer: u32 = PROBLEM.split("\n").map(|line| {
        ...
        if first_lhs <= second_lhs && first_rhs >= second_rhs {
            1
        } else if second_lhs <= first_lhs && second_rhs >= first_rhs {
            1
        } else {
            0
        }
